...
const EnterJobInfo = () => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (
        ...
        !selectedLocations.some((location) => item.workArea.includes(location))
      ) {
        isValid = false;
      }
      ...
      if (
        ...
        !selectedEducations.includes(item.edctn)
      ) {
        isValid = false;
      }
      if (
        selectedJobTypes.length > 0 &&
        !selectedJobTypes.some((jobType) => item.jobName.includes(jobType))
      ) {
        isValid = false;
      }
  ...
  const filteredAreas = useMemo(() => {
    return areas.filter((area) =>
      area.areaName.toLowerCase().includes(regionSearch.toLowerCase())
    );
  }, [areas, regionSearch]);

  const filteredJobTypes = useMemo(() => {
    return jobs.filter((job) =>
      job.jobScName.toLowerCase().includes(jobTypeSearch.toLowerCase())
    );
  }, [jobs, jobTypeSearch]);
      ...
      <section className="card card-admin">
        <div className="container py-4">
          <div className="d-flex justify-content-end">
            <Link to="/enter/jobPostingInsert" className="btn btn-primary">
              등록
            </Link>
          </div>
          ...
          <div className="list-container">
            <div className="list-header">
                            ...
                            onChange={(e) => {
                              setSelectedLocations((prev) =>
                                e.target.checked
                                  ? [...prev, area.areaName]
                                  : prev.filter(
                                      (location) => location !== area.areaName
                                    )
                              );
                            }}
                          />{" "}
              ...
              <div>
                ...
                <button onClick={clearAllFilter} className="btn btn-primary">
                  초기화
                </button>
              </div>
            </div>
              ...
              {paginatedItems.length > 0 ? (
                paginatedItems.map((item, idx) => (
                  <div key={idx} className="custom">
                    <div>
                      <Link to={`/board/detail/jobPosting/${item.jbpSq}`}>
                        {item.jbpTl}
                      </Link>
                    </div>
