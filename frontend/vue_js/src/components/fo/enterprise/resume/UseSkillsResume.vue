...
const emit = defineEmits(['update:isVisible', 'update:useSkillsData']);
...
onMounted(async () => {
    ...
    // 전달받은 기술로 자동 체크
watch(() => props.formattesUseSkil, async(newSkills) => {
...
const skillsSave = () => {
    console.log(selectedSkills.value.skillName)
    emit('update:useSkillsData', selectedSkills.value);
    close()
}
