<template>
  <div class="container mt-5">
    <section
      class="page-header page-header-modern bg-color-grey page-header-lg"
    >
      <div class="container">
        <div class="row">
          <div class="col-md-12 align-self-center p-static order-2 text-center">
            <h1 class="font-weight-bold text-dark">기업 계정정보 찾기</h1>
          </div>
        </div>
      </div>
    </section>

    <div class="loginSection">
      <div class="card mb-4">
        <div class="card-body">
          <div class="row">
            <div class="col">
              <div class="contact-form form-style-3">
                <!-- 아이디/비밀번호 찾기 버튼 -->
                <div class="row" v-if="!isPwResetMode">
                  <div class="col-6" style="padding: 0">
                    <button
                      class="btn btn-outline rounded-0 mb-2"
                      :class="{
                        'btn-primary': idPwCheck,
                        'btn-dark': !idPwCheck,
                      }"
                      @click="setIdPwCheck(true)"
                    >
                      아이디찾기
                    </button>
                  </div>
                  <div class="col-6" style="padding: 0">
                    <button
                      class="btn btn-outline rounded-0 mb-2"
                      :class="{
                        'btn-primary': !idPwCheck,
                        'btn-dark': idPwCheck,
                      }"
                      @click="setIdPwCheck(false)"
                    >
                      비밀번호찾기
                    </button>
                  </div>
                </div>
                <!-- 아이디/비밀번호 찾기 및 비밀번호 재설정 컴포넌트 -->
                <component
                  :is="
                    isPwResetMode
                      ? 'EnterPwReset'
                      : idPwCheck
                      ? 'EnterIdFind'
                      : 'EnterPwFind'
                  "
                  :entrprsSq="entrprsSq"
                  :handle-login-submit="handleLoginSubmit"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import { ref } from "vue";
import { showAlert } from "../../../../utill/utillModal";
import EnterIdFind from "./EnterIdFind.vue";
import EnterPwFind from "./EnterPwFind.vue";
import EnterPwReset from "./EnterPwReset.vue"; // 새로운 비밀번호 설정 컴포넌트
import { useRouter } from "vue-router";
import { api } from "../../../../axios";

export default {
  name: "EnterFindMain",
  components: {
    EnterIdFind,
    EnterPwFind,
    EnterPwReset,
  },
  setup() {
    const idPwCheck = ref(true);
    const isPwResetMode = ref(false); // 비밀번호 재설정 모드 상태
    const router = useRouter();
    const entrprsSq = ref(""); // 여기서 res 데이터를 저장할 변수

    const setIdPwCheck = (value) => {
      idPwCheck.value = value;
    };

    const handleLoginSubmit = async (data, formType) => {
      console.log("진입");
      console.log("엔터데이터", data);
      if (formType === "id") {
        try {
          console.log("엔터");
          const res = await api.$post("/enter/findId", data);
          console.log("결과는", res);
          if (res) {
            showAlert("고객님의 아이디는: " + res);
          } else {
            showAlert(
              "일치하는 회원정보가 없습니다.이름과 이메일을 확인 바랍니다"
            );
          }
        } catch (error) {
          if (error.response && error.response.status === 400) {
            showAlert(error.response.data); // 서버에서 받은 오류 메시지 표시
          } else {
            showAlert("알 수 없는 오류가 발생했습니다."); // 일반적인 오류 처리
          }
        }
      } else {
        // 비밀번호 찾기 로직
        try {
          const res = await api.$post("/enter/findPswrd", data);
          entrprsSq.value = res;
          isPwResetMode.value = true; // 비밀번호 재설정 모드로 전환
        } catch (error) {
          if (error.response && error.response.status === 400) {
            showAlert(error.response.data); // 서버에서 받은 오류 메시지 표시
          } else {
            showAlert("알 수 없는 오류가 발생했습니다."); // 일반적인 오류 처리
          }
        }
      }
    };

    return {
      idPwCheck,
      isPwResetMode, // 비밀번호 재설정 모드 상태 반환
      setIdPwCheck,
      handleLoginSubmit,
      router,
      entrprsSq,
    };
  },
};
</script>

<style scoped>
.btn-outline {
  font-size: 12px;
}
.card-body {
  width: 400px;
}
.row {
  padding: 20px;
}
.row .col-6 {
  text-align: center;
}
.btn-primary {
  background-color: #007bff;
  color: white;
}
.btn-secondary {
  background-color: #f8f9fa;
  color: #000;
}
.loginSection {
  display: flex;
  justify-content: center;
}
</style>
