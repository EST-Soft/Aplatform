<template>
        ...
        <div class="container">
          <div class="row">
            <div class="col-md-12 align-self-center p-static order-2 text-center">
              <h1 class="text-dark font-weight-bold text-8">프로젝트 등록 현황</h1>
            </div>
            <div class="col-md-12 align-self-center order-1">
              <ul class="breadcrumb d-block text-center">
                <li><a href="/">Home</a></li>
                <li class="active">프로젝트 등록 현황</li>
              </ul>
            </div>
          </div>
        </div>
    ...
    <div class="list-header">
      ...
      <!-- 검색 컴포넌트 -->
      <SearchProject @searchResult="handleSearchResult" />
    </div>
        ...
        <div v-for="(item, idx) in paginatedItems" :key="idx" class="custom">
          <table class="info-table" @click="goToDetailPage(item)">
            <tr>
              ...
              <td style="font-size:16px;">제목:<br>{{ item.prjctTtl }}</td>
              <td style="font-size:16px;">사용 스킬:<br>{{ item.prjctUseSkl }}</td>
              <td style="font-size:16px;">필수 스킬:<br>{{ item.prjctEsntlSkl }}</td>
              <td colspan="2" class="action-buttons-cell">
            ...
            <tr>
              <td style="font-size:14px;" v-html="formatWorkArea(item.prjctLctn)"></td>
              <td style="font-size:14px;">경력:<br><span style="font-size:18px;">{{ getCareerText(item.prjctEsntlCrr) }}</span></td>
              <td style="font-size:14px;">근무직군:<br><span style="font-size:18px;">{{ getEducationText(item.prjctTpy) }}</span></td>
              <td style="font-size:14px;" v-html="formatJobName(item.prjctWork)"></td>
              <td style="font-size:14px;">시작일:<br><span style="font-size:16px;">{{ item.prjctStrtDate }}</span></td>
              <td style="font-size:14px;">마감일:<br><span style="font-size:16px;">{{ item.prjctEndDate }}</span></td>
              <td>
                <div>
...
import SearchProject from '../../../components/fo/enterprise/SearchProject.vue';
  ...
  const addDeadline = (item) => {
    api.$put(`/project/deadline/${item.prjctSq}`)
      .then(() => {
  ...
  const goUpdatePage = (item) => {
    ...
    router.push(`/projectUpdate/${item.prjctSq}`);
  };
  
  const gotoapplyPage = (item) => {
    router.push(`/applys/apply-list/${item.prjctSq}`);
  }
  ...
  const deleteBoard = async (item) => {
    try {
      await api.$delete(`/project/${item.prjctSq}`);
      fetchItems();
      router.push('/enterProjectList');
    } catch (error) {
