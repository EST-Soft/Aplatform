    ...
    <div class="row">
      <div class="form-group col">
        ...
        <input
          ...
          autocomplete="off"
          v-model="formData.entrprsPicName"
          @focus="handleFocus"
          @blur="handleBlur"
        />
      </div>
      ...
      <div class="form-group col">
        ...
        <div class="d-flex align-items-center">
          <input
            ...
            autocomplete="off"
            v-model="formData.entrprsPicEml"
...
export default {
  ...
  setup(props) {
    ...
    const validateEmail = (entrprsPicEml) => {
      const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      return re.test(entrprsPicEml);
    };

    const requestAuthCode = () => {
      console.log(formData.value);
      if (!formData.value.entrprsPicEml) {
        emailError.value = "이메일을 입력해주세요.";
        return;
      } else if (!validateEmail(formData.value.entrprsPicEml)) {
    ...
    const onSubmit = (e) => {
      e.preventDefault();
      const data = {
        entrprsPicName: formData.value.entrprsPicName,
        entrprsId: formData.value.entrprsId,
        entrprsPicEml: formData.value.entrprsPicEml,
      };
      ...
      if (!formData.value.entrprsId) {
        showAlert("아이디를 입력해 주세요.");
        return;
      }
      if (!formData.value.entrprsPicName) {
